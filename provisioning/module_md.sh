...
wget -q -O /opt/software/gromacs-5.0.6.tar.gz ftp://ftp.gromacs.org/pub/gromacs/gromacs-5.0.6.tar.gz > /dev/null

(cd /opt/software && tar -xzf gromacs-5.0.6.tar.gz && cd gromacs-5.0.6 && mkdir build && cd build && \
    cmake ../ -DGMX_BUILD_OWN_FFTW=ON -DCMAKE_INSTALL_PREFIX=/opt/software/gromacs5 && make && \
    make install) > /dev/null

rm -rf /opt/software/gromacs-5.0.6 /opt/software/gromacs-5.0.6.tar.gz
